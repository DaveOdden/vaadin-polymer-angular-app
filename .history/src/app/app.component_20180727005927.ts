...
export class AppComponent {
  ...
  constructor( private StockApi: ApiService ) {
    StockApi.getStock().subscribe( data => {
      ...
      var twoAfterDecimal_endStockVal = this.endStockVal.toString().indexOf('.') > -1 ? this.endStockVal.toString().indexOf('.') + 3 : twoAfterDecimal_endStockVal;
      var twoAfterDecimal_netGains = this.netGains.toString().indexOf('.') + 3;

      console.log(this.endStockVal.toString().substring( 0, twoAfterDecimal_endStockVal ));
