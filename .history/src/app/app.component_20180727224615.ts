...
@Component({
  ...
  template: `
    ...
    <vaadin-context-menu style="padding: 1rem 4rem;">
      <template>
        <vaadin-list-box>
      ...
      <vaadin-item class="card-no-padding" style="max-width:15rem">
        <div class="space-m">
          ...
          <div *ngIf="!loadingStocks">
            ...
            <small *ngIf="isMarketOpen() == true" class="secondary-text">Current Price: \${{endStockValString}}</small>
            <small *ngIf="isMarketOpen() == false" class="secondary-text">Market Closed</small>
          </div>
        </div>

        <div class="contrast-5pct space-xs"><p style="font-size: .6375rem; padding-top: .12rem; line-height: 0; vertical-align: middle" class="secondary-text">Last Sync: {{lastReload}}</p></div>
      </vaadin-item>
    </vaadin-context-menu>`
})
