...
@Component({
  ...
  template: `
    ...
    <vaadin-item>
    <div>Today</div>
    <h2>\${{endStockValString}}</h2>
    </vaadin-item>
    
    <vaadin-item>
    <div>Net Gain</div>
    <h2>\${{netGainsString}}</h2>
    </vaadin-item>`
})
export class AppComponent {
  ...
  constructor( private StockApi: ApiService ) {
    StockApi.getStock().subscribe( data => {
      ...
      //this.endStockVal = parseInt(this.endStockVal.toString().substring( 0, (this.endStockVal.toString().indexOf('.') + 3)));

      var twoAfterDecimal_endStockVal = this.endStockVal.toString().indexOf('.') + 2;
      var twoAfterDecimal_netGains = this.netGains.toString().indexOf('.') + 2;
      
      console.log(twoAfterDecimal_endStockVal);
      console.log(twoAfterDecimal_netGains);

      this.endStockValString = this.endStockVal.toString().substring( 0, twoAfterDecimal_endStockVal );
      this.netGainsString = this.netGains.toString().substring( 0, twoAfterDecimal_netGains );
    });
  }
}