...
export class WealthComponent implements OnInit {
  ...
  private getStocks(): void {
    this.StockApi.getStock().subscribe( data => {
      ...
      console.log((parseFloat( this.apiData['4. close'] )) - (parseFloat( this.apiData['4. open'] )));
      this.pointVarianceTodayString = this.numberWithCommas( parseFloat( this.apiData['4. close'] ) - parseFloat( this.apiData['1. open'] ));
