...
export class WealthComponent implements OnInit {
  ...
  private prepareDataForUI( data ) : void {
    ...
    function storeHistoricalData( hsd: object[] ) {
      var counter = 0;
      for( let contextdate in data["Time Series (Daily)"] ) {
        if(counter <= 5) {
          hsd[counter]['date'] = contextdate;
          hsd[counter]['data'] = data["Time Series (Daily)"][contextdate];
          counter++;
        }
      }
    }
    ...
    function calculateVarianceInPrice() {
      return self.apiData ? (parseFloat( self.apiData['4. close'] ) - parseFloat( self.apiData['1. open'] )) : 0;
    }
    ...
    generateNeededDates()
    storeHistoricalData( this.historicalStockData );
