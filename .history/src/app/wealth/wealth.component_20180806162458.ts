  ...
  private getStocks(): void {
    this.StockApi.getStock().subscribe( data => {
      ...
      if(this.endStockVal != 0) {
        ...
        this.endStockValString = this.currencyFormat( parseInt( this.endStockVal.toString().substring( 0, twoAfterDecimal_endStockVal ) ) );
        this.netGainsString = this.currencyFormat( parseInt( this.netGains.toString().substring( 0, twoAfterDecimal_netGains ) ) );  
